...
class Path_Find(Field_Test):
    ...
    def start_end_location2(self,row,column):      
        if row<=0 or column<=0 or row>self.row_input or column>self.col_input:
            print "Rows go from 1 to %s and Columns go from 1 to %s" %(self.row_input,self.col_input)
            start=raw_input("Location of new point? (write value as row,column)")
            start_row,start_column=start.split(",")
            row,column=self.start_end_location2(int(start_row), int(start_column))
            return row,column
            
        if self.field2[row-1][column-1]!= 0 and self.field2[row-1][column-1] != "+":
                return row,column
    ...
    def choose_points(self):
        start=raw_input("Location of start point? (write value as row,column)")
        start_row,start_column=self.check_split(start)
        s_row,s_column=self.start_end_location2(int(start_row),int(start_column))
        self.path_values.append([s_row-1,s_column-1])
        self.field2[s_row-1][s_column-1]="+"
        
        end=raw_input("Location of end point? (write value as row,column)")
        end_row,end_column=self.check_split(end)
        e_row,e_column=self.start_end_location2(int(end_row),int(end_column))
        self.path_values.append([e_row-1,e_column-1])
        self.field2[e_row-1][e_column-1]="*"
        
    def check_int(self,point):
        row,column=self.check_split(point)
        try:
            return int(row),int(column)
        except ValueError:
            print "Please write in the desired format"
            start=raw_input("Location of new point? (write value as row,column)")
            row,column=self.check_point(start)
            print row,column
            return row,column
        
    def check_split(self,point):
        if "," not in point:
            print "Please write in the desired format"
            start=raw_input("Location of new point? (write value as row,column)")
            row,column=self.check_point(start)
    ...
    def value_spread(self):
        ...
        for i in range(len(self.value_cont)):
            for j in range(4):
                ...
                try:
                    if (self.field2[row][column]==" " or self.field2[row][column]=="+") and row>=0 and column>=0:
    ...
    def value_start(self):
        ...
        for i in range(4):
            try:
                if self.field2[self.path_values[1][0]+self.ops[i][0]][self.path_values[1][1]+self.ops[i][1]]==" " and self.path_values[1][0]+self.ops[i][0]>=0 and self.path_values[1][1]+self.ops[i][1]>=0:
                    self.field2[self.path_values[1][0]+self.ops[i][0]][self.path_values[1][1]+self.ops[i][1]]=value
                    self.value_cont.append([self.path_values[1][0]+self.ops[i][0],self.path_values[1][1]+self.ops[i][1],self.ops[i][2],value])
            except IndexError:
                pass
    ...
    def path(self,row,column):
        ...
        for i in range(len(self.ops)):
            try:
                if [row+self.ops[i][0],column+self.ops[i][1]]==self.path_values[1]:
    ...
    def execute(self):
        ...
        self.value_spread()
        #self.display_field2()
        self.path(self.path_values[0][0],self.path_values[0][1])
        self.clean_field()
        self.display_field()
    ...
    def clean_field(self):
        for i in range(len(self.path_values)):
            if i==1:
                self.field[self.path_values[1][0]][self.path_values[1][1]]="*"
            elif i==0:
                self.field[self.path_values[0][0]][self.path_values[0][1]]="+"
            else:
